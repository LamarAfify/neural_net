...
# change which labels to select, and the names of the variables
# Write code to filter both the training data and the test data down to two categories class = 3, class = 7


# selected_labels = [7, 3, 5]
# is_label_selected = train_data['class'].isin(selected_labels)
# filtered_df = train_data[is_label_selected]

# Separate the data (features) and the  classes
X_train = train_data.drop(
    'class', axis=1)  # Features (all columns except the first one)
...
# filtered_df = test_data[is_label_selected]
...
# 8<------------ cut here ---------------
neural_net_model = MLPClassifier(hidden_layer_sizes=(8),
                                 random_state=42,
                                 tol=0.005)
